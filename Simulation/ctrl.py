...
class Control:
    ...
    def attitude_control(self, quad, Ts):
        ...
        self.qd = utils.quatMultiply(
            self.qd_red, np.array([cos(self.yaw_w * np.arccos(q_mix[0])), 0, 0, sin(self.yaw_w * np.arcsin(q_mix[3]))])
        )
    ...
    def rate_control(self, quad, Ts):
        ...
        self.rateCtrl = (
            rate_P_gain * rate_error - rate_D_gain * quad.omega_dot
