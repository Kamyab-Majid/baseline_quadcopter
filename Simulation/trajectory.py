    ...
    def __init__(self, quad, ctrlType, trajSelect):
        ...
        if self.ctrlType == "xyz_pos":
            ...
            if self.averVel == 1:
                ...
                self.T_segment = (
                    np.sqrt(distance_segment[:, 0] ** 2 + distance_segment[:, 1] ** 2 + distance_segment[:, 2] ** 2)
                    / self.v_wp
                )
        ...
        self.sDes = np.hstack(
            (self.desPos, self.desVel, self.desAcc, self.desThr, self.desEul, self.desPQR, self.desYawRate)
        ).astype(float)
    ...
    def desiredState(self, t, Ts, quad):
        ...
        def pos_waypoint_arrived():
            ...
            if t == 0:
                ...
                distance_to_next_wp = (
                    (self.wps[self.t_idx, 0] - quad.pos[0]) ** 2
                    + (self.wps[self.t_idx, 1] - quad.pos[1]) ** 2
                    + (self.wps[self.t_idx, 2] - quad.pos[2]) ** 2
                ) ** (0.5)
        ...
        def yaw_follow():
            ...
            if (t == 0) or (t >= self.t_wps[-1]):
                ...
                if (
                    np.sign(self.desEul[2]) - np.sign(self.current_heading)
...
def testXYZposition(t):
    ...
    desThr = np.array([0.0, 0.0, 0.0])
    desEul = np.array([0.0, 0.0, 0.0])
    desPQR = np.array([0.0, 0.0, 0.0])
